...
let particlesInterval = null, lightningInterval = null, leavesInterval = null, birdInterval = null, fireflyInterval = null, shootingInterval = null;
function clearParticles(){
  ...
  $('birds-layer').innerHTML = ''; $('fireflies-layer').innerHTML = ''; $('shootingstars-layer').innerHTML = '';
}
function spawnBird(){
  const flockSize = Math.random() > 0.65 ? 2 + Math.floor(Math.random()*2) : 1;
  const baseTop = 8 + Math.random()*20;
  const baseDur = 14 + Math.random()*10;
  for(let i=0;i<flockSize;i++){
    const b = document.createElement('div'); b.className='bird';
    b.style.top = (baseTop + i*2.2)+'%';
    const dur = baseDur + i*0.6;
    b.style.animationDuration = dur+'s';
    b.style.animationDelay = (i*0.25)+'s';
    $('birds-layer').appendChild(b); setTimeout(()=>b.remove(), dur*1000+400);
  }
}
function spawnFirefly(){
  const f = document.createElement('div'); f.className='firefly';
  f.style.left = (8+Math.random()*84)+'%'; f.style.bottom = (2+Math.random()*15)+'%';
  const dur = 4+Math.random()*3;
  f.style.animationDuration = dur+'s';
  $('fireflies-layer').appendChild(f); setTimeout(()=>f.remove(), dur*1000+100);
}
function spawnShootingStar(){
  const s = document.createElement('div'); s.className='shooting-star';
  s.style.left = (15+Math.random()*55)+'%'; s.style.top = (5+Math.random()*25)+'%';
  $('shootingstars-layer').appendChild(s); setTimeout(()=>s.remove(), 1500);
}
function buildTreeline(){
  const wrap = $('treeline'); wrap.innerHTML = '';
...
function buildFog(){
  const wrap = $('fog-layer'); wrap.innerHTML = '';
  for(let i=0;i<3;i++){
    const b = document.createElement('div'); b.className='fog-band';
    b.style.left = (i*30-15)+'%'; b.style.animationDelay = (i*6)+'s'; b.style.animationDuration = (22+i*5)+'s';
    wrap.appendChild(b);
  }
}
function applyVisuals(profile, isDay){
  ...
  $('sun-rays').classList.toggle('show', isDay && profile.key === 'clear');
  $('storm-vignette').classList.toggle('show', profile.key === 'storm');
  $('ground-mist').classList.toggle('show', profile.key === 'rain' || profile.key === 'storm');
  clearParticles();
  if(lightningInterval){ clearInterval(lightningInterval); lightningInterval=null; }
  if(!settings.animations){
    $('sun-rays').classList.remove('show'); $('storm-vignette').classList.remove('show'); $('ground-mist').classList.remove('show');
    return;
  }
  ...
  if(profile.key === 'rain' || profile.key === 'storm'){
    const heavy = profile.key === 'storm';
    particlesInterval = setInterval(()=>{
      const dropsPerTick = heavy ? 4 : 3;
      for(let i=0;i<dropsPerTick;i++){
        const d = document.createElement('div'); d.className='drop';
        const big = Math.random() > 0.5;
        d.style.left = Math.random()*100+'%';
        d.style.setProperty('--dx', (heavy ? 60+Math.random()*40 : 25+Math.random()*25)+'px');
        d.style.width = (big ? 3 : 1.5)+'px';
        d.style.height = (big ? (22+Math.random()*10) : (12+Math.random()*8))+'px';
        d.style.opacity = (big ? .8 : .45) + Math.random()*.2;
        const dur = big ? .38+Math.random()*.15 : .55+Math.random()*.25;
        d.style.animation = `fallDrop ${dur}s linear forwards`;
        sky.appendChild(d); setTimeout(()=>d.remove(), dur*1000+150);
      }
    }, heavy ? 28 : 38);
  }
  if(profile.key === 'snow'){
    particlesInterval = setInterval(()=>{
      for(let i=0;i<2;i++){
        ...
        f.style.setProperty('--dx1', (10+Math.random()*22)+'px');
        f.style.fontSize = (big ? (16+Math.random()*10) : (8+Math.random()*7))+'px';
        f.style.opacity = (big ? .85 : .45) + Math.random()*.15;
        f.style.filter = big ? 'none' : 'blur(.4px)';
        const dur = big ? 4+Math.random()*2.5 : 6+Math.random()*4;
        f.style.animation = `fallFlakeDrift ${dur}s ease-in-out forwards`;
        sky.appendChild(f); setTimeout(()=>f.remove(), dur*1000+150);
      }
    }, 90);
  }
  ...
  /* paisaje vivo: pájaros de día, luciérnagas y estrellas fugaces de noche */
  $('fog-layer').classList.toggle('show', profile.key === 'fog');
  if(isDay && ['clear','clouds','fog'].includes(profile.key)){
    birdInterval = setInterval(spawnBird, 6000 + Math.random()*6000);
  }
  if(!isDay && ['clear','clouds','fog'].includes(profile.key)){
    fireflyInterval = setInterval(spawnFirefly, 900);
  }
  if(!isDay && profile.key === 'clear'){
    shootingInterval = setInterval(spawnShootingStar, 15000 + Math.random()*25000);
  }
}
const styleTag = document.createElement('style');
styleTag.textContent = `
@keyframes fallDrop{ from{transform:translate(0,0);} to{transform:translate(var(--dx,20px),110vh);} }
...
@keyframes fallFlakeDrift{
  0%{ transform:translate(0,0) rotate(0deg); }
  25%{ transform:translate(var(--dx1,15px), 25vh) rotate(80deg); }
  50%{ transform:translate(calc(var(--dx1,15px) * -1), 52vh) rotate(180deg); }
  75%{ transform:translate(var(--dx1,15px), 78vh) rotate(260deg); }
...
async function geocodeCity(name){
  ...
  return { name:r.name, country:r.country, countryCode:r.country_code, region:r.admin1, latitude:r.latitude, longitude:r.longitude };
}
...
async function reverseGeocode(lat, lon){
  try{
    ...
    return {
      ...
      region: data.principalSubdivision || '', country: data.countryName || '', countryCode: data.countryCode || '',
      latitude:lat, longitude:lon
    };
  }catch(e){ return { name:'Tu ubicación', region:'', country:'', countryCode:'', latitude:lat, longitude:lon }; }
}
/* Elevación del terreno vía Open-Meteo Elevation API. Gratuita, sin clave. */
...
/* Bandera y capital del país vía REST Countries. Gratuita, sin clave. Con caché en memoria para no repetir peticiones. */
const countryInfoCache = {};
async function getCountryInfo(countryCode){
  if(!countryCode) return null;
  if(countryInfoCache[countryCode]) return countryInfoCache[countryCode];
  try{
    const res = await fetch(`https://restcountries.com/v3.1/alpha/${countryCode}?fields=flag,capital`);
    const data = await res.json();
    const info = { flag: data.flag || '', capital: data.capital?.[0] || '' };
    countryInfoCache[countryCode] = info;
    return info;
  }catch(e){ return null; }
}
...
/* Carga bandera + capital (REST Countries) y elevación (Open-Meteo) sin bloquear el resto del render */
async function loadLocationExtras(place){
  const [elevation, countryInfo] = await Promise.all([
    getElevation(place.latitude, place.longitude),
    getCountryInfo(place.countryCode)
  ]);
  const parts = [];
  if(countryInfo?.flag) parts.push(countryInfo.flag);
  const base = [place.region, place.country].filter(Boolean).join(', ');
  if(base) parts.push(base);
  if(elevation != null) parts.push(`⛰️ ${Math.round(elevation)} m`);
  if(countryInfo?.capital && countryInfo.capital !== place.name) parts.push(`🏛️ ${countryInfo.capital}`);
  if(parts.length) $('location-sub').textContent = parts.join('  ·  ');
}
...
/* ==================================================================
   ...
   La clave se lee de config.js (window.SKYMOOD_CONFIG.nasaApiKey).
   Si no hay config.js o no tiene clave, usa 'DEMO_KEY' (muy limitada, ~30/hora).
   Se guarda en caché por día para no gastar peticiones de más.
   ================================================================== */
const NASA_API_KEY = (window.SKYMOOD_CONFIG && window.SKYMOOD_CONFIG.nasaApiKey) || 'DEMO_KEY';
...
/* ==================================================================
   Pausa mental — Open Trivia DB (gratuita, sin clave)
   Pequeña pregunta de cultura general para usar durante los descansos del Pomodoro.
   ================================================================== */
...
async function loadTrivia(){
  $('trivia-question').textContent = 'Cargando pregunta...';
  $('trivia-options').innerHTML = ''; $('trivia-meta').textContent = '';
  try{
    const res = await fetch('https://opentdb.com/api.php?amount=1&type=multiple');
    const data = await res.json();
    const q = data.results?.[0];
    if(!q) throw new Error('sin pregunta');
    $('trivia-meta').textContent = `${decodeHtml(q.category)} · ${q.difficulty}`;
    $('trivia-question').textContent = decodeHtml(q.question);
    const options = shuffle([...q.incorrect_answers.map(decodeHtml), decodeHtml(q.correct_answer)]);
    const correct = decodeHtml(q.correct_answer);
    const wrap = $('trivia-options');
    options.forEach(opt=>{
      const b = document.createElement('button'); b.className = 'trivia-opt'; b.textContent = opt;
      b.addEventListener('click', ()=>{
        wrap.querySelectorAll('.trivia-opt').forEach(o=>{
          o.disabled = true;
  ...
  }catch(e){ $('trivia-question').textContent = 'No se pudo cargar la pregunta. Inténtalo de nuevo.'; }
}
$('trivia-refresh').addEventListener('click', loadTrivia);
...
window.addEventListener('resize', ()=>{ clearTimeout(resizeTimer); resizeTimer = setTimeout(buildTreeline, 300); });
buildSoundButtons();
applyTheme();
renderStats();
renderGoal();
